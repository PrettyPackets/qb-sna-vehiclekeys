...
local function AttemptPoliceAlertPS(Vehicle)
    if not AlertSend then
        local chance = Config.PoliceAlertChance
        if GetClockHours() >= 1 and GetClockHours() <= 6 then
            chance = Config.PoliceNightAlertChance
...
local function LockpickDoor(isAdvanced)
    ...
    if Config.PS_UI then
        ...
        end, Config.PS_UI_Game.numCircles, Config.PS_UI_Game.numTime)
    else
...
function lockpickFinish(success)
    ...
    if success then
        ...
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        if Config.PS_Dispatch then
            AttemptPoliceAlertPS(vehicle)
        else
    ...
    for k, location in pairs(Config.KeyMasterLocations) do
        ...
        if Config.Target then
            ...
            exports.interact:AddLocalEntityInteraction({
                entity = KeyMasters[index].ped,
                name = 'keyped_' .. index, -- optional
                id = 'keyped_' .. index, -- needed for removing interactions
                distance = 6.0, 
                interactDst = 3.0, 
                ignoreLos = true,
                ...
                options = {
                    {
                        label = Lang:t("info.givekey"),
                        action = function()
                            TriggerEvent("qb-vehiclekeys:client:GiveKeyMenu")
                        end,
                    },
                    {
                        label = Lang:t("info.resetlocks"),
                        action = function()
                            TriggerEvent("qb-vehiclekeys:client:ResetLocksMenu")
                        end,
                    },
                }
            })
